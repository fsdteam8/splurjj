            ...
            {images.map((imageUrl, index) => (
              <CarouselItem key={`${posts.id}-${index}`}>
                <div className="relative cursor-pointer group" onClick={() => handleImageClick(index)}>
                  ...
                  {/* Image counter badge */}
                  {images.length > 1 && (
                    <div className="absolute top-4 right-4 bg-black/50 text-white px-2 py-1 rounded-full text-sm font-medium border-2 border-blue-500">
...
//                     width={1200}
//                     height={600}
//                     className="aspect-[2/1] w-full object-contain transition-transform duration-200 group-hover:scale-[1.02] border-2 border-red-500"
//                     onError={(e) => {
